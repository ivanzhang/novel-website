...
test('novel 详情页脚本应包含首屏转化区、目录预览和同类推荐容器', async () => {
  ...
  assert.match(html, /handleCoverError/);
  assert.match(html, /data-class-name/);
  assert.match(html, /renderFallbackCover/);
  assert.match(html, /className}-fallback/);
  assert.match(html, /INITIAL_CHAPTER_BATCH_SIZE/);
  assert.match(html, /loadMoreChapters/);
  assert.match(html, /collapseChapterList/);
  assert.match(html, /chapter-list-load-more/);
  assert.doesNotMatch(html, /chapter-list-full/);
  assert.match(html, /loadMemberCenter/);
  assert.match(html, /renderMembershipConversionCard/);
  assert.match(html, /novel-membership-card/);
  assert.match(html, /member-center/);
});
