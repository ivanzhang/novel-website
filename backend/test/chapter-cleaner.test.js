...
test('sanitizeChapterContent 应删除重复连写的水印 token', () => {
  const raw = [
    '“老大，这小子好像不行了！要不bq118♜ccbq118♜ccbq118♜cc”',
    '“不能死，我还不能死，不能死！bq118♜ccbq118♜ccbq118♜ccbq118♜cc”夏宇无数次挣扎着。'
  ].join('\n');
  ...
  assert.equal(cleaned, [
    '“老大，这小子好像不行了！要不”',
    '“不能死，我还不能死，不能死！”夏宇无数次挣扎着。'
  ].join('\n'));
});

test('sanitizeChapterContent 应删除 token 后粘连的尾随字符', () => {
  const raw = [
    '“真的不是性冷淡？”萧遥脸色有些不确定的出声问道bq330ヽccw书友整~理提~供',
    '“激将法对我没用，请你离开bq330ヽcc”韩瑞闻言，撇了撇嘴。'
  ].join('\n');

  const cleaned = sanitizeChapterContent(raw);

  assert.equal(cleaned, [
    '“真的不是性冷淡？”萧遥脸色有些不确定的出声问道',
    '“激将法对我没用，请你离开”韩瑞闻言，撇了撇嘴。'
  ].join('\n'));
});
