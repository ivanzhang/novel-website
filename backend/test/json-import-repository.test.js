...
test('db.transaction 包裹的 upsertNovel + replaceChapters 应在失败时整体回滚', () => {
  const db = createTestDb();
  try {
    const novelId = seedNovel(db, {
      title: '旧标题',
    ...
    const { upsertNovel, replaceChapters } = loadRepository();
    const importInTransaction = db.transaction(() => {
      const importedNovelId = upsertNovel({
        source_site: 'https://0732.bqg291.cc',
        source_book_id: '2530',
        title: '万相之王',
        author: '天蚕土豆',
      ...
      replaceChapters(importedNovelId, [
        {
          chapter_number: 1,
          title: '第1章',
          content_file_path: 'chapters/2530/1.json',
